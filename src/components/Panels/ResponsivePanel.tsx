  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Viewport Preview</h3>
        ...
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-600 mb-2">Current Viewport:</div>
      ...
      {/* Common Breakpoints */}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Common Breakpoints</h3>
        <div className="grid grid-cols-2 gap-2">
          {commonBreakpoints.map((breakpoint) => (
            <button
              key={breakpoint.name}
              onClick={() => addMediaQuery(breakpoint.query, breakpoint.label)}
              className="px-3 py-2 text-xs font-medium text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-md transition-colors border border-blue-200 text-left"
            >
              <div className="font-semibold">{breakpoint.label}</div>
              <div className="text-xs text-gray-500">{breakpoint.query}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Orientation Queries */}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Orientation</h3>
        <div className="grid grid-cols-2 gap-2">
      ...
      {/* Feature Queries */}
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Feature Queries</h3>
        <div className="grid grid-cols-2 gap-2">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-800">Active Media Queries</h3>
          <button
        ...
        {mediaQueries.length === 0 ? (
          ...
          <div className="space-y-2">
            {mediaQueries.map((query) => (
              <div key={query.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg">
                <div className="flex-1">
                  <div className="font-medium text-sm">{query.name}</div>
                  <div className="text-xs text-gray-500 font-mono">{query.condition}</div>
                </div>
                <button
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-800">Container Queries</h3>
          <button
        ...
        <div className="p-3 bg-blue-50 rounded-lg">
          <div className="text-xs text-blue-800 font-medium mb-1">Container Queries</div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Custom Media Query Builder</h3>
        <div className="p-4 bg-gray-50 rounded-lg space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Property</label>
              <select className="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent">
          ...
          <button className="w-full px-3 py-2 bg-purple-600 text-white rounded text-xs hover:bg-purple-700 transition-colors">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-800">Responsive Design Tips</h3>
