...
export const InteractionsPanel: React.FC<InteractionsPanelProps> = ({
  ...
  const CursorPreview = ({ cursorType }: { cursorType: string }) => (
    <div 
      className="w-8 h-8 bg-purple-100 rounded border-2 border-purple-300 flex items-center justify-center text-xs font-medium cursor-pointer"
            ...
            {cursorOptions.slice(0, 16).map((cursorType) => (
              <button
                ...
                className={`p-1 rounded transition-colors ${
                  cursor === cursorType ? 'bg-purple-200' : 'hover:bg-gray-200'
                }`}
                title={cursorType}
              >
